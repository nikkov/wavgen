...
{
/* ****************************************************************************
PFAFFT - Functions to performPrime Factor (PFA) FFT's, in place

npfa		return valid n for Complex-to-ComplexPFA
npfao		return optimal n for Complex-to-ComplexPFA
npfar		return valid n for real-to-Complex/Complex-to-realPFA
npfaro		return optimal n for real-to-Complex/Complex-to-realPFA

pfacc		1DPFA Complex to Complex
pfacr		1DPFA Complex to real
pfarc		1DPFA real to Complex

pfamcc		multiplePFA Complex to Complex

pfa2cc		2DPFA Complex to Complex
pfa2cr		2DPFA Complex to real
pfa2rc		2DPFA real to Complex
...
COREDSP_API void PfaFft ( Complex *cz, ulong n, int isign )
{
	...
	REAL *z = (REAL*)cz;
	ulong j0, j1, j2, j3, j4, j5, j6, j7, j8, j9, j10, j11, j12, j13, j14, j15, jt;
	ulong nleft, jfax, ifac, jfac, jinc, jmax, ndiv, m, mm = 0, mu = 0, l;
	double t1r,t1i,t2r,t2i,t3r,t3i,t4r,t4i,t5r,t5i,
	...
	for ( jfax = 0; jfax < NFAX; jfax++ )
	{
		...
		if ( ifac == 7 )
		{
			...
			for ( l = 0; l < m; l++ )
			{
				...
				z[j0] = REAL(z[j0]+t1r+t2r+t3r);
				z[j0+1] = REAL(z[j0+1]+t1i+t2i+t3i);
				z[j1] = REAL(y1r-y6i);
				z[j1+1] = REAL(y1i+y6r);
				z[j2] = REAL(y2r-y5i);
				z[j2+1] = REAL(y2i+y5r);
		...
		if ( ifac == 11 )
		{
			...
			for ( l = 0; l < m; l++ )
			{
				...
				z[j0] = REAL(z[j0]+t1r+t2r+t3r+t4r+t5r);
				z[j0+1] = REAL(z[j0+1]+t1i+t2i+t3i+t4i+t5i);
				z[j1] = REAL(y1r-y10i);
				z[j1+1] = REAL(y1i+y10r);
				z[j2] = REAL(y2r-y9i);
				z[j2+1] = REAL(y2i+y9r);
				z[j3] = REAL(y3r-y8i);
				z[j3+1] = REAL(y3i+y8r);
		...
		if (ifac==13) {
			...
			for ( l = 0; l < m; l++ )
			{
				...
				z[j0] = REAL(z[j0]+t1r+t2r+t3r+t4r+t5r+t6r);
				z[j0+1] = REAL(z[j0+1]+t1i+t2i+t3i+t4i+t5i+t6i);
				z[j1] = REAL(y1r-y12i);
				z[j1+1] = REAL(y1i+y12r);
				z[j2] = REAL(y2r-y11i);
				z[j2+1] = REAL(y2i+y11r);
				z[j3] = REAL(y3r-y10i);
				z[j3+1] = REAL(y3i+y10r);
				z[j4] = REAL(y4r-y9i);
				z[j4+1] = REAL(y4i+y9r);
				z[j5] = REAL(y5r-y8i);
				z[j5+1] = REAL(y5i+y8r);
				...
				z[j9+1] = REAL(y4i-y9r);
				z[j10] = REAL(y3r+y10i);
				z[j10+1] = REAL(y3i-y10r);
				z[j11] = REAL(y2r+y11i);
				z[j11+1] = REAL(y2i-y11r);
				z[j12] = REAL(y1r+y12i);
				z[j12+1] = REAL(y1i-y12r);
				jt = j12+2;
				j12 = j11+2;
				j11 = j10+2;
				j10 = j9+2;
		...
		if ( ifac == 16 ) {
			...
			for (l=0; l<m; l++) {
				...
				y15i = t38i+t37i;
				z[j0] = REAL(t31r+t32r);
				z[j0+1] = REAL(t31i+t32i);
				z[j1] = REAL(y1r-y15i);
				z[j1+1] = REAL(y1i+y15r);
				z[j2] = REAL(y2r-y14i);
				z[j2+1] = REAL(y2i+y14r);
				z[j3] = REAL(y3r-y13i);
				z[j3+1] = REAL(y3i+y13r);
				z[j4] = REAL(y4r-y12i);
				z[j4+1] = REAL(y4i+y12r);
				z[j5] = REAL(y5r-y11i);
				z[j5+1] = REAL(y5i+y11r);
				z[j6] = REAL(y6r-y10i);
				z[j6+1] = REAL(y6i+y10r);
				z[j7] = REAL(y7r-y9i);
				z[j7+1] = REAL(y7i+y9r);
				z[j8] = REAL(t31r-t32r);
				z[j8+1] = REAL(t31i-t32i);
				z[j9] = REAL(y7r+y9i);
				z[j9+1] = REAL(y7i-y9r);
				z[j10] = REAL(y6r+y10i);
				z[j10+1] = REAL(y6i-y10r);
				z[j11] = REAL(y5r+y11i);
				z[j11+1] = REAL(y5i-y11r);
				z[j12] = REAL(y4r+y12i);
				z[j12+1] = REAL(y4i-y12r);
				z[j13] = REAL(y3r+y13i);
				z[j13+1] = REAL(y3i-y13r);
				z[j14] = REAL(y2r+y14i);
				z[j14+1] = REAL(y2i-y14r);
				z[j15] = REAL(y1r+y15i);
				z[j15+1] = REAL(y1i-y15r);
				jt = j15+2;
				j15 = j14+2;
				j14 = j13+2;
				j13 = j12+2;
...
******************************************************************************/
COREDSP_API void pfacr ( Complex *cz, REAL *rz, ulong n, int isign )
{
...
Also, see notes and references for function pfacc.
******************************************************************************/
COREDSP_API void pfarc ( REAL *rz, Complex *cz, ulong n, int isign )
{
	ulong i,ir,ii,jr,ji,no2;
	REAL *z;
	double tempr,tempi,sumr,sumi,difr,difi;
...
ComputationalPhysics, v. 58, p. 283-299.

Temperton, C., 1988, A new set of minimum-add rotated
rotated dft modules: Journal of ComputationalPhysics,
v. 75, p. 190-198.
******************************************************************************/
COREDSP_API void pfamcc ( Complex *cz, ulong n, ulong nt, ulong k, ulong kt, int isign )
{
	static int kfax[] = { 16,13,11,9,8,7,5,4,3,2 };
	REAL *z=(REAL*)cz;
	ulong j0, j1, j2, j3, j4, j5, j6, j7, j8, j9, j10, j11, j12, j13, j14, j15;
	ulong nleft, jfax, ifac, jfac, iinc, imax, ndiv, m, mm = 0, mu = 0, l, istep, jstep,
...
v. 75, p. 190-198.
******************************************************************************/
COREDSP_API void pfa2cc ( Complex *cz, ulong idim, ulong n1, ulong n2, int isign )
{
	ulong n,nt,k,kt;
...
j++ after the i1++.
**************************************************************************** */
COREDSP_API void pfa2cr ( Complex *cz, REAL *rz, ulong idim, ulong n1, ulong n2, int isign )
{
	ulong i1,i2,j,k,it,jt,kt,n,nt,itmul,itinc;
	...
	if (idim==1) {
		...
		pfa2cc( (Complex*)z, 1, n1/2, n2, isign );

		/* else, if transforming dimension 2 */
	} else {

		/* transform as Complex elements */
		pfa2cc((Complex*)z, 2, n1, n2/2, isign );
...
Also, see notes and references for function pfa2cc.
******************************************************************************/
COREDSP_API void pfa2rc ( REAL *rz, Complex *cz, ulong idim, ulong n1, ulong n2, int isign )
{
	ulong i1,i2,j,k,it,jt,kt,n,nt,itmul,itinc;
	REAL *z,*temp,tempr,tempi,sumr,sumi,difr,difi;
	REAL wr,wi,wpr,wpi,wtemp,theta;
...
{
	ulong i;
	for ( i = 0; i < NTAB - 1 && nctab[i].n < n; ++i );
	return nctab[i].n == n;
}

COREDSP_API ulong GetPfaFftLength( ulong n, int isign )
{
	for ( ulong i = 1; i < NTAB - 1; ++i )
		if( nctab[i].n >= n )
			return isign > 0 ? nctab[i].n : nctab[i - 1].n;

	return isign > 0 ? nctab[NTAB - 1].n : nctab[0].n;
}
